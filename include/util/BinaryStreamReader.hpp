...
/**
 * @brief Utility for reading multi-byte values in big-endian (network byte order) from binary streams.
 * 
 * This namespace provides simple, type-safe helpers to read 16-bit and 32-bit integers from
 * unaligned network buffers without triggering alignment warnings or aliasing issues.
 * All functions read in big-endian (network) byte order, as per DNS/mDNS specifications.
 */
