...
inline DnsHeaderHost parse_dns_header(const uint8_t *buf, size_t len) {
  if (len < DnsProtocol::kHeaderSize)
    throw std::invalid_argument("DNS buffer too small for header");
  DnsHeaderHost h;
...
// Parse a DNS QNAME at buffer[offset]. Returns the dotted name and advances
// offset
/**
 ...
 */
inline std::pair<std::string, size_t> parse_qname(const uint8_t *buf,
  ...
  while (pos < len) {
    ...
    if ((label_len & DnsProtocol::kQnamePointerPrefix) ==
        DnsProtocol::kQnamePointerPrefix) {
      if (pos + 1 >= len) {
        throw std::invalid_argument("Compressed QNAME pointer truncated");
      }
      uint16_t pointer = ((static_cast<uint16_t>(
                               label_len & DnsProtocol::kQnamePointerOffsetMask)
                           << 8) |
                          buf[pos + 1]);
      if (pointer >= len) {
