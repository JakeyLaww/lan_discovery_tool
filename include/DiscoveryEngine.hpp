...
/**
 * @brief High-level discovery engine that broadcasts mDNS queries and listens
 * for responses.
 */
...
public:
  ...
  bool run(uint32_t poll_interval_ms = 5000,
...
private:
  ...
  void send_mdns_questions(const std::vector<MdnsProbeRequest> &questions,
                           const char *success_log, const char *error_context);
