 ...
 *
 * MdnsMessageDecoder encapsulates the logic to parse a complete DNS message
 * from its wire format (binary buffer) into structured data types. It
 * consolidates repeated parsing patterns and eliminates code duplication in the
 * procedural Parser.cpp.
 *
 * This class is intended to coexist with the procedural Parser.cpp functions;
 * the procedural functions can delegate to this class for backward
 * compatibility while allowing new code to use the cleaner class-based API.
 *
 * Example:
...
private:
  ...
  /**
   * @brief Parse a section of resource records (answers, authorities, or
   * additionals).
   *
   * @param count    Number of resource records to parse in this section.
   * @param pos      Current buffer position; updated to point after the last
   * parsed record.
   * @return std::vector<MdnsResourceRecord> Parsed resource records.
   */
  std::vector<MdnsResourceRecord> parse_record_section(uint16_t count,
                                                       size_t &pos);

  /**
   * @brief Parse a single resource record starting at the given position.
   *
   * @param pos  Current buffer position; updated to point after the parsed
   * record.
...
public:
   ...
   *
   * @param buf     Pointer to buffer containing wire format DNS message. Must
   * not be nullptr.
