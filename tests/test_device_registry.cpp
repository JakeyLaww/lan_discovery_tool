...
static ResourceRecordView ptr_meta(const std::string &service, uint64_t ts,
                                   uint32_t ttl = 4500) {
  return ResourceRecordView{MdnsNames::kDnsSdMetaService, DnsType::PTR, service,
                            ttl, ts};
}
...
static ResourceRecordView srv_record(const std::string &instance,
                                     const std::string &target, uint64_t ts) {
  return ResourceRecordView{instance, DnsType::SRV,
...
void test_meta_ptr_adds_service_type() {
  ...
  if (require_snap(registry, "10.0.0.5")
          .service_types.count("_airplay._tcp.local") != 1) {
...
void test_service_ptr_adds_instance() {
  ...
  ev.records.push_back(ptr_instance("_airplay._tcp.local",
                                    "Living Room._airplay._tcp.local", 2000));

  require_apply(registry, ev);
  if (require_snap(registry, "10.0.0.5")
          .instances.count("Living Room._airplay._tcp.local") != 1) {
...
void test_srv_sets_host() {
  ...
  ev.records.push_back(
      srv_record("Living Room._airplay._tcp.local", "tv.local", 3000));
...
void test_instance_ptr_adds_service_type() {
  ...
  ev.records.push_back(ptr_instance("_device-info._tcp.local",
                                    "Mac._device-info._tcp.local", 7000));

  require_apply(registry, ev);
  if (require_snap(registry, "10.0.0.5")
          .service_types.count("_device-info._tcp.local") != 1) {
...
int main() {
  ...
  try {
    ...
    std::cout << std::endl
              << "✓ All device registry tests passed!" << std::endl;
    return 0;
