...
static ResourceRecordView make_view(const std::string &owner, uint16_t type,
                                    const std::string &rdata) {
  return ResourceRecordView{owner, type, rdata, 120, 1000};
}

void test_meta_ptr_relevant() {
  const auto rec = make_view(MdnsNames::kDnsSdMetaService, DnsType::PTR,
                             "_airplay._tcp.local");
...
void test_arpa_ptr_excluded() {
  const auto rec =
      make_view("79.128.10.in-addr.arpa", DnsType::PTR, "host.example");
  assert(!MdnsNames::is_audit_relevant_record(rec));
  std::cout << "  ✓ arpa PTR excluded passed" << std::endl;
}

void test_kerberos_txt_excluded() {
  const auto rec = make_view("_kerberos.khuhlinas-macbook-pro-7.local",
                             DnsType::TXT, "txtvers=1");
  assert(!MdnsNames::is_audit_relevant_record(rec));
...
void test_service_type_from_name() {
  ...
  assert(MdnsNames::service_type_from_name("_airplay._tcp.local") ==
         "_airplay._tcp.local");
