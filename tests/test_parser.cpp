...
void test_parse_basic_header() {
  ...
  // Construct a minimal DNS header: ID=0x1234, Flags=response, Questions=1,
  // Answers=2
  uint8_t buffer[12] = {
  ...
  if (header.transaction_id != 0x1234 ||
      header.flags != DnsProtocol::kFlagsResponse || header.questions != 1 ||
      header.answer_rrs != 2 || header.authority_rrs != 0 ||
...
void test_parse_qname_simple() {
  std::cout << "Testing parse_qname with simple uncompressed name..."
            << std::endl;
...
void test_parse_questions() {
  ...
  buffer[pos++] = 0; // terminator
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x01; // Type = 1 (A)
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x01; // Class = 1 (IN)
...
void test_parse_full_message() {
  ...
  buffer[1] = 0x00; // ID
  buffer[2] = 0x84;
  buffer[3] = 0x00; // Flags = 0x8400 (response)
  buffer[4] = 0x00;
  buffer[5] = 0x01; // Questions = 1
  buffer[6] = 0x00;
  buffer[7] = 0x01; // Answers = 1
  buffer[8] = 0x00;
  buffer[9] = 0x00; // Authority = 0
  buffer[10] = 0x00;
  buffer[11] = 0x00; // Additional = 0
  ...
  buffer[pos++] = 0; // terminator
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x01; // Type = A
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x01; // Class = IN
  ...
  buffer[pos++] = 0x01; // Class = IN
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x00; // TTL high bytes
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x78; // TTL = 120
  buffer[pos++] = 0x00;
  buffer[pos++] = 0x04; // RDLENGTH = 4
  buffer[pos++] = 192;
  buffer[pos++] = 168;
  buffer[pos++] = 1;
  buffer[pos++] = 1;
...
void test_rdata_formatters() {
  ...
  {
    uint8_t ptr_data[] = {0x04, '_',  'i', 't', 'u', 'n', 'e', 's',
                          '_',  0x04, '_', 't', 'c', 'p', 0x00};
    MdnsResourceRecord rr;
    rr.type = DnsType::PTR;
