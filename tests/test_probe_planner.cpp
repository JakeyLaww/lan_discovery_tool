...
static DiscoveryEvent make_ptr_event(const std::string& owner, const std::string& rdata) {
    DiscoveryEvent ev;
    ev.timestamp_ms = 1;
...
void test_meta_ptr_enqueues_service_type_ptr() {
    ...
    planner.observe(make_ptr_event("_services._dns-sd._udp.local", "_airplay._tcp.local"));
...
void test_service_type_ptr_enqueues_srv_txt() {
    MdnsProbePlanner planner;
    planner.observe(make_ptr_event("_airplay._tcp.local", "Living Room._airplay._tcp.local"));
...
void test_dedup_same_target_not_queued_twice() {
    MdnsProbePlanner planner;
    const auto ev = make_ptr_event("_services._dns-sd._udp.local", "_airplay._tcp.local");
    planner.observe(ev);
...
void test_cooldown_blocks_requeue() {
    ...
    const auto ev = make_ptr_event("_services._dns-sd._udp.local", "_http._tcp.local");
    planner.observe(ev);
    planner.drain_ready(8);
    planner.observe(ev);
