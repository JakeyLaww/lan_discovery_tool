...
DiscoveryEngine::DiscoveryEngine(
    ...
    : socket(std::make_unique<NetworkSocket>()), logger(std::move(logger_)),
      packet_interpreter(interpreter
...
bool DiscoveryEngine::start(const std::string &interface_name) {
  ...
  try {
    ...
    logger->error(std::string("Failed to bind to mDNS multicast group: ") +
                  ex.what());
    return false;
  }
...
void DiscoveryEngine::send_mdns_questions(
    const std::vector<MdnsProbeRequest> &questions, const char *success_log,
...
void DiscoveryEngine::broadcast_meta_browse() {
  send_mdns_questions(
      {MdnsProbeRequest{MdnsNames::kDnsSdMetaService, DnsType::PTR}},
...
void DiscoveryEngine::send_probe_batch(const MdnsProbeBatch &batch) {
  ...
  send_mdns_questions(batch, probe_log_msg.c_str(),
                      "Failed to send probe query: ");
}
...
void DiscoveryEngine::set_event_sink(EventSinkPtr s) { sink = std::move(s); }
...
void DiscoveryEngine::worker_loop() {
  while (running.load()) {
    std::vector<uint8_t> buffer;
    std::string src_ip;
    if (!packet_queue_->wait_pop(buffer, src_ip,
...
bool DiscoveryEngine::run(uint32_t poll_interval_ms,
  ...
  try {
    ...
    poller_thread =
        std::thread(&DiscoveryEngine::poller_loop, this, poll_interval_ms);
  ...
  if (dropped_packets > 0) {
    logger->warn("Dropped " + std::to_string(dropped_packets) +
                 " packets due to queue limit (" +
                 std::to_string(kMaxPacketQueueSize) + ")");
  }

  logger->info("mDNS session: " + std::to_string(responses_seen_) +
               " responses, " + std::to_string(queries_seen_) +
               " queries from other hosts, " + std::to_string(sink_events_) +
               " discovery events emitted");
  if (responses_seen_ == 0 && queries_seen_ > 0) {
    logger->warn("Heard other hosts' mDNS queries but no responses to this "
                 "scanner. Ensure Bonjour "
                 "devices are awake, or pin the LAN NIC with -I eth1 (see "
                 "startup interfaces).");
  }
