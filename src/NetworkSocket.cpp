 ...
 */
void NetworkSocket::bind_multicast(uint16_t port,
...
#ifdef SO_REUSEPORT
  if (setsockopt(sock_fd, SOL_SOCKET, SO_REUSEPORT, (char *)&reuse,
                 sizeof(reuse)) < 0) {
  ...
  if (!interface_ipv4.empty()) {
    ...
    if (setsockopt(sock_fd, IPPROTO_IP, IP_MULTICAST_IF, &send_iface,
                   sizeof(send_iface)) < 0) {
      cleanup_and_throw("setsockopt(IP_MULTICAST_IF)");
    }
  }

  // Receive on all local interfaces (INADDR_ANY). Pinning imr_interface to one
  // NIC breaks multicast ingress on WSL2 mirrored networking; egress uses
  // send_iface above.
  ...
  int loop = 0;
  if (setsockopt(sock_fd, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop)) <
      0) {
 ...
 */
ssize_t NetworkSocket::send_packet(const std::vector<uint8_t> &data,
                                   const std::string &dest_ip,
                                   uint16_t dest_port) {
  if (sock_fd < 0) {
    throw std::system_error(ENOTSOCK, std::generic_category(),
                            "send_packet: socket not initialized");
  }
  ...
  ssize_t sent = sendto(sock_fd, data.data(), data.size(), 0,
...
/**
 * @brief Receive a UDP packet into the provided buffer and return the sender
 * IP.
 *
 * @param buffer Vector to be filled with the received payload (resized to
 * actual length).
 * @param sender_ip Output string containing the sender IPv4 address in dotted
 * form.
 * @return ssize_t Number of bytes received.
 */
ssize_t NetworkSocket::receive_packet(std::vector<uint8_t> &buffer,
                                      std::string &sender_ip) {
  if (sock_fd < 0) {
    throw std::system_error(ENOTSOCK, std::generic_category(),
                            "receive_packet: socket not initialized");
  }
  ...
  if (inet_ntop(AF_INET, &sender_addr.sin_addr, ipbuf, sizeof(ipbuf)) ==
      nullptr) {
...
void NetworkSocket::set_receive_timeout(uint32_t milliseconds) {
  if (sock_fd < 0) {
    throw std::system_error(ENOTSOCK, std::generic_category(),
                            "set_receive_timeout: socket not initialized");
  }
  struct timeval tv;
