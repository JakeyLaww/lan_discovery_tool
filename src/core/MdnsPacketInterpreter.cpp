...
void append_records(std::vector<ResourceRecordView> &out,
  ...
  for (const auto &rr : rrs) {
    ResourceRecordView view =
        make_record_view(rr, packet, packet_len, observed_at_ms);
    if (!is_displayable_record(view))
      continue;
...
MdnsParsedMessage
MdnsPacketInterpreter::decode_packet(const std::vector<uint8_t> &buffer) {
  MdnsMessageDecoder decoder(buffer.data(), buffer.size());
  return decoder.decode();
}

DiscoveryEvent MdnsPacketInterpreter::create_discovery_event(
    const std::string &src_ip, const std::vector<uint8_t> &packet,
  ...
  std::unordered_set<std::string> seen;
  append_records(ev.records, parsed_packet.answers, wire, wire_len,
                 ev.timestamp_ms, seen);
  append_records(ev.records, parsed_packet.authorities, wire, wire_len,
                 ev.timestamp_ms, seen);
  append_records(ev.records, parsed_packet.additionals, wire, wire_len,
                 ev.timestamp_ms, seen);
...
void MdnsPacketInterpreter::log_undisplayable_response(
    const std::string &src_ip, const MdnsParsedMessage &parsed_packet) {
  std::ostringstream oss;
  oss << "response from " << src_ip << " with "
      << parsed_packet.header.answer_rrs << " answers, 0 displayable records";
  logger->debug(oss.str());
}

void MdnsPacketInterpreter::log_packet_summary(
    const std::string &src_ip, const std::vector<uint8_t> &buffer,
    const MdnsParsedMessage &parsed_packet) {
  if (!verbose_)
    return;
  ...
  summary << "Parsed: " << src_ip << " " << parsed_packet.header.questions
          << " Q, " << parsed_packet.header.answer_rrs << " A, "
