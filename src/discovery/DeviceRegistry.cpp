...
bool add_service_type(std::set<std::string> &service_types,
...
bool DeviceRegistry::merge_records(DeviceSnapshot &snap,
                                   const DiscoveryEvent &ev) {
  ...
  for (const auto &rec : ev.records) {
    ...
    if (rec.owner_name == MdnsNames::kDnsSdMetaService &&
        rec.type == DnsType::PTR &&
        MdnsNames::is_service_type_name(rec.rdata_text)) {
      if (insert_if_new(snap.service_types, rec.rdata_text))
    ...
    if (MdnsNames::is_service_type_name(rec.owner_name) &&
      ...
      if (insert_if_new(snap.instances, rec.rdata_text))
        profile_changed = true;
      if (add_service_type(snap.service_types, rec.rdata_text))
        profile_changed = true;
    }

    if (rec.type == DnsType::SRV || rec.type == DnsType::TXT) {
      if (add_service_type(snap.service_types, rec.owner_name))
        profile_changed = true;
    }
...
bool DeviceRegistry::apply(const DiscoveryEvent &ev) {
  if (ev.src_ip.empty() || ev.records.empty())
    return false;
...
std::string DeviceRegistry::format_summary(const std::string &src_ip) const {
  ...
  if (!snap->service_types.empty()) {
    oss << "\n  services: "
        << join_set(snap->service_types, snap->service_types.size());
  }
  if (!snap->instances.empty()) {
    oss << "\n  instances: "
        << join_set(snap->instances, kMaxInstancesInSummary);
  }
