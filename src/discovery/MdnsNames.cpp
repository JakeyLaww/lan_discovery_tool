...
bool is_service_type_name(const std::string &name) {
  if (!has_mdns_service_suffix(name))
    return false;
  if (is_dns_sd_meta_service(name))
    return false;
...
std::string service_type_from_name(const std::string &name) {
  ...
  if (transport_pos == std::string::npos || transport_pos == 0)
    return {};

  const size_t dot_before_service = name.rfind('.', transport_pos - 1);
  if (dot_before_service == std::string::npos)
    return {};
...
bool is_audit_relevant_record(const ResourceRecordView &rec) {
  switch (rec.type) {
  case DnsType::PTR:
    if (contains_arpa(rec.owner_name) || contains_arpa(rec.rdata_text))
      return false;
    if (rec.owner_name == kDnsSdMetaService &&
        is_service_type_name(rec.rdata_text)) {
      return true;
    }
    return is_service_type_name(rec.owner_name) &&
           is_instance_name(rec.rdata_text);
