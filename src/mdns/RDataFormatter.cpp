...
std::string ARecordFormatter::format(const std::vector<uint8_t>& rdata) const {
    if (rdata.size() != 4) {
...
std::string AAAARecordFormatter::format(const std::vector<uint8_t>& rdata) const {
    if (rdata.size() != 16) {
...
std::string PTRRecordFormatter::format(const std::vector<uint8_t>& rdata) const {
    if (rdata.empty()) {
        return "(empty PTR record)";
    }
    try {
        auto [ptr_name, _] = MDNS::parse_qname(rdata.data(), rdata.size(), 0);
        return ptr_name;
    } catch (...) {
...
std::string SRVRecordFormatter::format(const std::vector<uint8_t>& rdata) const {
    if (rdata.size() < 6) {
    ...
    if (rdata.size() > 6) {
        try {
            auto [target, _] = MDNS::parse_qname(rdata.data(), rdata.size(), 6);
            oss << " target=" << target;
        } catch (...) {
...
std::string TXTRecordFormatter::format(const std::vector<uint8_t>& rdata) const {
    std::ostringstream oss;
    size_t pos = 0;
    bool first = true;
...
std::string GenericRecordFormatter::format(const std::vector<uint8_t>& rdata) const {
    std::ostringstream oss;
