...
std::string ARecordFormatter::format(const RDataFormatContext &ctx) const {
  ...
  oss << static_cast<int>(rdata[0]) << "." << static_cast<int>(rdata[1]) << "."
      << static_cast<int>(rdata[2]) << "." << static_cast<int>(rdata[3]);
  return oss.str();
}
...
std::string PTRRecordFormatter::format(const RDataFormatContext &ctx) const {
  ...
  try {
    auto [ptr_name, _] =
        MDNS::parse_qname(ctx.message, ctx.message_len, ctx.rdata_offset);
    return ptr_name;
  } catch (...) {
    return "(malformed PTR record)";
  }
...
std::string SRVRecordFormatter::format(const RDataFormatContext &ctx) const {
  ...
  if (rdata.size() > 6 && ctx.message && ctx.message_len > 0) {
    try {
      ...
      auto [target, _] =
          MDNS::parse_qname(ctx.message, ctx.message_len, target_offset);
      oss << " target=" << target;
    } catch (...) {
...
std::string TXTRecordFormatter::format(const RDataFormatContext &ctx) const {
  ...
  while (pos < rdata.size()) {
    ...
    ++pos;
    if (pos + str_len > rdata.size())
      break;
    if (!first)
      oss << " ";
    oss << std::string(reinterpret_cast<const char *>(rdata.data() + pos),
                       str_len);
    pos += str_len;
    first = false;
  }
...
std::string
GenericRecordFormatter::format(const RDataFormatContext &ctx) const {
  const auto &rdata = *ctx.rdata;
  std::ostringstream oss;
...
RDataFormatterPtr get_rdata_formatter(uint16_t type) {
  switch (type) {
  case DnsType::A:
    return std::make_shared<ARecordFormatter>();
  case DnsType::AAAA:
    return std::make_shared<AAAARecordFormatter>();
  case DnsType::PTR:
    return std::make_shared<PTRRecordFormatter>();
  case DnsType::SRV:
    return std::make_shared<SRVRecordFormatter>();
  case DnsType::TXT:
    return std::make_shared<TXTRecordFormatter>();
  default:
    return std::make_shared<GenericRecordFormatter>();
  }
}
